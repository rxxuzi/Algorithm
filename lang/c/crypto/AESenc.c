...
void MixColumns(uint8_t state[][4]) {
    ...
    for (int i = 0; i < s; i++) {
        ...
        state[0][i] ^= gf_mul[col[1]][1];
        state[0][i] ^= col[2];
        state[0][i] ^= col[3];

        state[1][i] = col[0];
        state[1][i] ^= gf_mul[col[1]][0];
        state[1][i] ^= gf_mul[col[2]][1];
        state[1][i] ^= col[3];

        state[2][i] = col[0];
        state[2][i] ^= col[1];
        state[2][i] ^= gf_mul[col[2]][0];
        state[2][i] ^= gf_mul[col[3]][1];

        state[3][i] = gf_mul[col[0]][1];
        state[3][i] ^= col[1];
        state[3][i] ^= col[2];
        state[3][i] ^= gf_mul[col[3]][0];
    }
}
...
void Encrypt(uint8_t in[], uint8_t out[], uint32_t key[], int keysize) {
    ...
    for(int round = 1; round <= 9; round++) {
        SubBytes(state); // バイトの置換
        ShiftRows(state); // 行のシフト
        MixColumns(state); // 列の混合
        AddRoundKey(state, &key[round * 4]); // ラウンドキーの加算
    }
