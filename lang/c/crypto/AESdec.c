...
void AddRoundKey(uint8_t state[][4], uint32_t w[]) {
    ...
    for (int i = 0; i < s; i++) {
        for (int j = 0 ; j < s ; j++ ) subkey[j]    = w[i] >> (24 - 8 * j);
        for (int j = 0 ; j < s ; j++ ) state[j][i] ^= subkey[j];
    }
}
...
void InvMixColumns(uint8_t state[][4]) {
    ...
    for (int i = 0; i < s; i++) {
        ...
        state[0][i] ^= gf_mul[col[1]][3];
        state[0][i] ^= gf_mul[col[2]][4];
        state[0][i] ^= gf_mul[col[3]][2];
        state[1][i] = gf_mul[col[0]][2];
        state[1][i] ^= gf_mul[col[1]][5];
        state[1][i] ^= gf_mul[col[2]][3];
        state[1][i] ^= gf_mul[col[3]][4];
        state[2][i] = gf_mul[col[0]][4];
        state[2][i] ^= gf_mul[col[1]][2];
        state[2][i] ^= gf_mul[col[2]][5];
        state[2][i] ^= gf_mul[col[3]][3];
        state[3][i] = gf_mul[col[0]][3];
        state[3][i] ^= gf_mul[col[1]][4];
        state[3][i] ^= gf_mul[col[2]][2];
        state[3][i] ^= gf_mul[col[3]][5];
    }
}
...
void Decrypt(uint8_t in[], uint8_t out[], uint32_t key[], int keysize) {
    ...
    // Copy input array to state array
    for (int i = 0; i < s; i++) {
        for (int j = 0; j < s; j++) {
            state[i][j] = in[i + j * s];
        }
